...
def extract_user_info(detail_page_url, payload, cookie):
	# Required_info
	required_user_info = set(
		['ctl1_first_name', 'ctl1_m_name', 'ctl1_last_name', 'ctl1_license_no', 'ctl1_license_type',
		 'ctl1_status', 'ctl1_issue_date', 'ctl1_expiry', 'ctl1_last_ren'])
...
pagination_list = [re.search(r"(datagrid_results\$.*,)", link).group()[:-2] for link in href_tags if
				   "__doPostBack('datagrid_results" in link]
...
write_to_csv(all_users_info)+ "{}".format(status)+ "{}".format(original_issued_date) + "{}".format(expiry) +"{}".format(renewed) +   "\n"